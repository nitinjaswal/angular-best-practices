...
export class CatalogRepositoryService {
  ...
  enroll(classId:string): Observable<any> {
    ...
    if (this.userRepository.currentUser.classes.includes(classId))
      return throwError(() => new Error("Already enrolled"));

    this.userRepository.currentUser.classes.push(classId);
  ...
  drop(classId:string): Observable<any> {
    ...
    this.userRepository.currentUser.classes = this.userRepository.currentUser.classes.filter(
      (c:string) => c !== classId
    );
